...
export class PaymentComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  GridProps: GridModel.IGrid = {
    ...
    column: [
      ...
      {
        field: 'pelanggan_code',
        headerName: 'Kode Plgn',
        class: 'text-xs',
        width: '100px',
      },
      {
            ...
            if (args) {
              const payload = {
                invoice_date: this._utilityService.onFormatDate(
                  new Date(args.invoice_date),
                  'MMM yyyy'
                ),
                due_date: this._utilityService.onFormatDate(
  ...
  ngOnInit(): void {
    ...
    this.getAll({
      payment_date: this._utilityService.onFormatDate(new Date(), 'yyyy-MM-DD'),
    });
    this.getAllProduct();
    ...
    if (Object.keys(queryParams).length) {
      ...
      if (queryParams['state'] == 'cash') {
        ...
        this.getAllInvoice(
          ...
          true,
          this.QueryParams.id_invoice
        );
      }

      if (queryParams['state'] == 'invoice') {
  ...
  private getAllProduct() {
    this._productService
      ...
      .subscribe((result) => {
        if (result) {
          const index = this.AdvancedFilterProps.findIndex(
            (item) => item.id == 'product_name'
          );
  ...
  private getAllPelanggan(query?: any, isSet?: true, value?: number) {
    this._pelangganService
      .getAll(query, true)
      .pipe(takeUntil(this.Destroy$))
      .subscribe((result) => {
        if (result) {
          const index = this.FormProps.fields.findIndex(
            (item) => item.id == 'id_pelanggan'
          );
          this.FormProps.fields[index].dropdownProps.options = result.data;

          if (isSet) {
            this.FormComps.FormGroup.get('id_pelanggan')?.setValue(value);
          }
        }
      });
  }
  ...
  private getAllInvoice(
    ...
    this._invoiceService
      ...
      .subscribe((result) => {
        if (result) {
          const index = this.FormProps.fields.findIndex(
            (item) => item.id == 'id_invoice'
          );
          this.FormProps.fields[index].dropdownProps.options = result.data;

          if (is_set) {
            setTimeout(() => {
              this.getAllPelanggan(null, true, parseInt(id_pelanggan));

              this.FormComps.FormGroup.get('id_invoice')?.setValue(
                parseInt(id_invoice as any)
              );

              const invoice = result.data.find(
                (item) => item.id_invoice == parseInt(id_invoice as any)
              );
              const payload = {
                invoice_date: this._utilityService.onFormatDate(
  ...
  private getAllPaymentMethodManual(
    ...
    this._settingCompanyService
      ...
      .subscribe((result) => {
        if (result.status) {
          const index = this.FormProps.fields.findIndex(
            (item) => item.id == 'id_payment_method_manual'
          );
          this.FormProps.fields[index].dropdownProps.options = result.data;

          if (is_set) {
            const index = result.data.findIndex(
              (item: any) => item.payment_method === payment_method_manual
            );

            if (index > -1) {
              this.FormComps.FormGroup.get(
                'id_payment_method_manual'
              )?.setValue(result.data[index].id_payment_method_manual);
            }
          }
        }
      });
  ...
  handleBackToList() {
    ...
    } else if (this.FromInvoice) {
      this._router.navigateByUrl(`/tagihan`);
    } else {
      this.FormState == 'detail'
        ? this.FormDetailComps.onResetForm()
        : this.FormComps.onResetForm();

      const indexManual = this.FormDetailProps.fields.findIndex(
        (item) => item.id == 'id_payment_method_manual'
      );
      this.FormDetailProps.fields[indexManual].hidden = false;

      const indexPaymentMethod = this.FormDetailProps.fields.findIndex(
    ...
    if (args.payment_provider != 'MANUAL' && this.FormState == 'detail') {
      ...
      setTimeout(() => {
        args.due_date = args.due_date
          ? this._utilityService.onFormatDate(
              new Date(args.due_date),
              'DD MMMM yyyy'
            )
          : 'Invoice Telah Dibatalkan';
        args.payment_date = new Date(args.payment_date);
        args.invoice_date = this._utilityService.onFormatDate(
          new Date(args.invoice_date),
          'MMMM yyyy'
        );
      ...
      setTimeout(() => {
        ...
        const indexManual = this.FormProps.fields.findIndex(
          (item) => item.id == 'id_payment_method_manual'
        );
  ...
  onToolbarClicked(args: any): void {
    ...
    if (args.type == 'edit') {
      if (args.data.payment_provider == 'MANUAL') {
        this.onRowDoubleClicked(args.data);
      } else {
    ...
    if (args.type == 'print') {
      if (args.data.payment_status == 'PAID') {
        this._router.navigateByUrl(
          `/print-out/pos?id_payment=${args.data.id_payment}`
        );
      } else {
        this._messageService.clear();
        this._messageService.add({
          severity: 'warn',
          summary: 'Oops',
          detail: 'Aksi ini hanya untuk PAID payment',
        });
      }
    }
    ...
    if (args.type == 'cancel') {
      if (args.data.payment_provider == 'MANUAL') {
        ...
        this._messageService.add({
          ...
          summary: 'Oops',
          detail:
            'Aksi ini hanya untuk payment yang tidak melalui Payment Gateway',
        });
      }
    }
  }
      ...
      .subscribe((result) => {
        if (result.status) {
          ...
          this._messageService.add({
            severity: 'success',
            summary: 'Success!',
            detail: 'Data canceled succesfully',
          });
          this.getAll(this.GridQueryParams);
        }
      });
