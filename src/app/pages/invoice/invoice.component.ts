...
export class InvoiceComponent implements OnInit, AfterViewInit, OnDestroy {
      ...
      fields: [
            ...
            if (args) {
              if (!args.product_id) {
                ...
                this._messageService.add({
                  severity: 'warning',
                  summary: 'Oops',
                  detail: 'Pelanggan Ini Belum Berlanggan Produk Apapun',
                });
              } else {
                this.FormComps.FormGroup.get('id_pelanggan_product')?.setValue(
                  args.id_pelanggan_product
                );
                this.FormComps.FormGroup.get('id_product')?.setValue(
                  args.product_id
                );
                this.FormComps.FormGroup.get('product_name')?.setValue(
                  args.product_name
                );
                this.FormComps.FormGroup.get('price')?.setValue(
                  args.product_price
                );
        ...
        },
        {
          ...
          onChange: (args: any) => {
            if (args) {
              ...
              this.FormComps.FormGroup.get('diskon_rupiah')?.setValue(
                price * (args / 100)
              );
              this.countSubtotalForm();
            }
        ...
        },
        {
          ...
          onChange: (args: any) => {
            if (args) {
              const price = this.FormComps.FormGroup.get('price')?.value;
              this.FormComps.FormGroup.get('diskon_percentage')?.setValue(
    ...
    if (Object.keys(queryParams).length) {
      ...
      if (queryParams['state'] == 'add') {
        ...
        this.getAllPelanggan(
          { search: queryParams['full_name'] },
          true,
          true,
          queryParams['id_pelanggan']
        );
      }
          ...
          this._activatedRoute.queryParams
            ...
            .subscribe((result) => {
              if (result && result['state'] == 'add') {
                this.FormComps.FormGroup.get('id_pelanggan')?.setValue(
                  parseInt(result['id_pelanggan'])
                );
              }
            });

          if (is_set) {
            const data: any = result.data.find(
              (item) => item.id_pelanggan == id_pelanggan
            );

            if (data) {
              this.FormComps.FormGroup.get('id_pelanggan_product')?.setValue(
                data.id_pelanggan_product
              );
              this.FormComps.FormGroup.get('id_product')?.setValue(
                data.product_id
              );
              ...
              this.FormComps.FormGroup.get('invoice_date')?.setValue(
                new Date()
              );
              this.countSubtotalForm();
        ...
        .subscribe((result) => {
          if (result.status) {
            ...
            this._messageService.add({
              severity: 'success',
              summary: 'Berhasil',
              detail: 'Job Generate Invoice Telah Berjalan',
            });
          }
        });
    }
        ...
        .subscribe((result) => {
          if (result.status) {
            ...
            this._messageService.add({
              severity: 'success',
              summary: 'Berhasil',
              detail: 'Job Send Message Telah Berjalan',
            });
          }
        });
    }
        ...
        .subscribe((result) => {
          if (result.status) {
            ...
            this._messageService.add({
              severity: 'success',
              summary: 'Berhasil',
              detail: 'Pesan Whatsapp Berhasil Dikirim',
            });
          }
        });
    }
    ...
    if (args.type == 'kirim pesan lunas') {
      if (args.data.id_payment) {
        ...
        this._messageService.add({
          severity: 'warn',
          summary: 'Oops',
          detail: 'Tagihan Belum Dibayarkan',
        });
      }
    }

    if (args.type == 'buat pembayaran') {
      if (args.data.invoice_status == 'PENDING') {
        console.log('detail =>', args.data);
        this._router.navigateByUrl(
          `/pembayaran?state=invoice&id_invoice=${args.data.id_invoice}&id_pelanggan=${args.data.id_pelanggan}`
        );
      } else {
        this._messageService.clear();
        this._messageService.add({
          severity: 'warn',
          summary: 'Oops',
          detail: 'Invoice Tidak Dalam Status Pending',
        });
      }
    }
  }
  ...
  saveData(data: any) {
    ...
    data.due_date = new Date(
      this._utilityService.onFormatDate(new Date(data.due_date), 'yyyy-MM-DD')
    ).toISOString();
      ...
      .subscribe((result) => {
        if (result.status) {
          ...
          this._messageService.add({
            severity: 'success',
            summary: 'Success!',
            detail: 'Data deleted succesfully',
          });
          this.getAll(this.GridQueryParams);
        }
      });
